...
function relax(mesh, scl::Float64, m::Matrix{Float64}, Ms::Float64, Aexc::Float64, Aan::Float64, uan::Vector{Float64}, Hap::Vector{Float64}, dt::Float64, maxTorque::Float64 ,giro::Float64, damp::Float64=1, precession::Bool=true, maxAtt::Int32=10_000)
    #= 
        Inspired by mumax3, this relax function minimizes the energy by time iteration of the
        Landau-Lifshitz equation. Repeats until the torque term is small        
    =#
    ...
    while div > maxTorque && it < maxAtt
        ...
        if mod(it,100) < 1
            println("Relaxing for (ns): ",it*(dt*1e9),"; Average torque: ",div)
        end
...
#=
    This is commented so that Hysteresis.jl can include the functions and not run the
    main(). If you just want to use a single minimization by steepest descent, you can
...
#     Hap::Vector{Float64}  = [0,800e3,0] # A/m

#     # Convergence criteria
#     maxAtt::Int32 = 1_000         # max number of iterations for the LL solver
